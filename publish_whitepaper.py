...
async def publish():
    ...
    # First, check if the article exists to avoid SQLite IntegrityError, or delete it and save to ensure clean slate
    conn = db.get_connection()
    cursor = conn.cursor()
    # Check for existing article with this exact title
    cursor.execute("SELECT id FROM articles WHERE title = ?", (title.strip(),))
    existing = cursor.fetchone()
    if existing:
        print(f"Article with title '{title}' already exists in SQLite DB. Deleting to avoid duplicate entries and re-saving.")
        cursor.execute("DELETE FROM articles WHERE title = ?", (title.strip(),))
        conn.commit()
    conn.close()

    # Save initial draft of this article in local DB
    db.save_article(title, vertical, article_content, category, ref_age)
    print(f"Saved fresh record of '{title}' in local DB.")

    wix_item_id = None
    if MOCK_MODE:
        wix_item_id = "wix-item-mock-67890"
    ...
    print(f"\nUpdated SQLite Database. Title: '{title}', Wix ID: '{wix_item_id}', Status: 'draft'")
